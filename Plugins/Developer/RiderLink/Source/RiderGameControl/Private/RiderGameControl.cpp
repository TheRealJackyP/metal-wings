...
FRiderGameControlActionsCache::FRiderGameControlActionsCache()
{
	...
	TSharedPtr<FBindingContext> PlayWorldContext = FInputBindingManager::Get().
		GetContextByName(PlayWorldContextName);
...
public:
	FRiderGameControl(rd::Lifetime Lifetime,
	                  const JetBrains::EditorPlugin::RdEditorModel& Model,
...
private:
	...
	void ScheduleModelAction(
		TFunction<void(const JetBrains::EditorPlugin::RdEditorModel&)> Action);
...
void FRiderGameControl::SendRequestSucceed(int RequestID)
{
	...
	ScheduleModelAction([=](const RdEditorModel& EditorModel)
	{
		EditorModel.get_notificationReplyFromEditor().fire(RequestSucceed(RequestID));
	});
}

void FRiderGameControl::SendRequestFailed(int RequestID,
                                          JetBrains::EditorPlugin::NotificationType
                                          Type,
                                          const FString& Message)
{
	using namespace JetBrains::EditorPlugin;
...
void FRiderGameControl::RequestPlayWorldCommand(const FCachedCommandInfo& CommandInfo,
                                                int RequestID)
{
	using namespace JetBrains::EditorPlugin;
...
FRiderGameControl::FRiderGameControl(rd::Lifetime Lifetime,
                                     const JetBrains::EditorPlugin::RdEditorModel&
                                     Model,
                                     FRiderGameControlActionsCache& ActionsCache) :
	Actions(ActionsCache), Model(Model)
{
	...
	Lifetime->bracket(
		[this]()
		{
			...
			{
				ScheduleModelAction([](const RdEditorModel& model)
				{
			...
			SingleStepPIEHandle = FEditorDelegates::SingleStepPIE.AddLambda(
				[this](const bool)
				{
					ScheduleModelAction([](const RdEditorModel& model)
					{
			...
			OnObjectPropertyChangedHandle =
				FCoreUObjectDelegates::OnObjectPropertyChanged.AddLambda(
					...
					{
						ULevelEditorPlaySettings* PlayInSettings = GetMutableDefault<
						...
						ScheduleModelAction([PlayModeNew](const RdEditorModel& Model)
						{
	...
	// Subscribe to model
	ScheduleModelAction([Lifetime, this](const RdEditorModel& Model)
	{
		     ...
		     .advise(Lifetime, [this](int requestID)
		             {
			             ...
			             check(PlayInSettings);
			             const EPlayModeType PlayMode = PlayInSettings->
				             LastExecutedPlayModeType;

			             RequestPlayWorldCommand(Actions.PlayModeCommands[PlayMode],
			                                     requestID);
		             }
		     );
		     ...
		     .advise(Lifetime, [this](int32_t mode)
		             {
			             ...
			             check(PlayInSettings);
			             const FPlaySettings NewSettings =
				             FPlaySettings::UnpackFromMode(mode);
	...
	// Initial sync.
	const ULevelEditorPlaySettings* PlayInSettings = GetDefault<
		ULevelEditorPlaySettings>();
	check(PlayInSettings);
	const FPlaySettings Settings = RetrieveSettings(PlayInSettings);
	playMode = FPlaySettings::PackToMode(Settings);

	ScheduleModelAction([lambdaPlayMode=playMode](const RdEditorModel& Model)
	{
...
void FRiderGameControlModule::StartupModule()
{
	...
	RiderLinkModule.ViewModel(
		ModuleLifetime,
		[&](rd::Lifetime ModelLifetime, const RdEditorModel& Model)
		{
			ModelLifetime->add_action([&]() { GameControl.Reset(); });
			GameControl = MakeUnique<FRiderGameControl>(
				ModelLifetime, Model, *ActionsCache);
		}
	);
