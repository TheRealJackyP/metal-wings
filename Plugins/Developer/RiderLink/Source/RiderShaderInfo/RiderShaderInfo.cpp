...
void FRiderShaderInfoModule::StartupModule()
{
	...
	const FString MappingFile = FPaths::Combine(Plugin->GetBaseDir(),
	                                            TEXT("Intermediate"),
	                                            TEXT("FileSystemMappings.ini"));
	const FString TmpMappingFile = FPaths::Combine(Plugin->GetBaseDir(),
	                                               TEXT("Intermediate"),
	                                               TEXT("~FileSystemMappings.ini"));
	...
	const TArray<FString> Mappings = [&ShaderMappings]()-> TArray<FString>
	{
		...
		{
			Result.Add(FString::Printf(TEXT("%s=%s"), *Pair.Key,
