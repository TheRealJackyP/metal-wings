...
namespace JetBrains::EditorPlugin
{
	...
	LogMessageInfo::LogMessageInfo(ELogVerbosity::Type type_, FString category_,
		...
		, type_(std::move(type_)), category_(std::move(category_)),
		time_(std::move(time_))
	{
		initialize();
	}
	...
	void LogMessageInfo::write(rd::SerializationCtx& ctx, rd::Buffer& buffer) const
	{
		...
		rd::Polymorphic<std::decay_t<decltype(category_)>>::write(
			ctx, buffer, category_);
		buffer.write_nullable<rd::DateTime>(time_,
		                                    [&ctx, &buffer](
		                                    const rd::DateTime& it) mutable -> void
		                                    {
	...
	// identify
	// getters
	const ELogVerbosity::Type& LogMessageInfo::get_type() const
	{
		return type_;
	}

	const FString& LogMessageInfo::get_category() const
	{
		return category_;
	}

	const rd::optional<rd::DateTime>& LogMessageInfo::get_time() const
	{
		return time_;
	}
	...
	// intern
	// equals trait
	bool LogMessageInfo::equals(const ISerializable& object) const
	{
		const auto& other = dynamic_cast<const LogMessageInfo&>(object);
		if (this == &other)
		{
			return true;
		}
