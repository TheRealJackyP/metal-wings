...
namespace JetBrains::EditorPlugin
{
	...
	{
		auto frames_ = buffer.read_array<
			TArray, ScriptCallStackFrame, FDefaultAllocator>(
			[&ctx, &buffer]() mutable
			{
	...
	void ScriptCallStack::write(rd::SerializationCtx& ctx, rd::Buffer& buffer) const
	{
		buffer.write_array<TArray, ScriptCallStackFrame, FDefaultAllocator>(frames_,
			[&ctx, &buffer](const ScriptCallStackFrame& it) mutable -> void
			{
	...
	// identify
	// getters
	const TArray<rd::Wrapper<ScriptCallStackFrame>>& ScriptCallStack::get_frames() const
	{
		return frames_;
	}
	...
	// intern
	// equals trait
	bool ScriptCallStack::equals(const ISerializable& object) const
	{
		const auto& other = dynamic_cast<const ScriptCallStack&>(object);
		if (this == &other)
		{
			return true;
		}
	...
	{
		size_t __r = 0;
		__r = __r * 31 + (contentDeepHashCode(get_frames()));
		return __r;
	}
