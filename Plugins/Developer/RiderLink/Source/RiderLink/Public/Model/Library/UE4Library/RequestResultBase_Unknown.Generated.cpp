...
namespace JetBrains::EditorPlugin
{
	...
	RequestResultBase_Unknown::RequestResultBase_Unknown(
		int32_t requestID_, rd::RdId unknownId_, rd::Buffer::ByteArray unknownBytes_) :
		RequestResultBase(std::move(requestID_)),
		IUnknownInstance(std::move(unknownId_))
	...
	// reader
	RequestResultBase_Unknown RequestResultBase_Unknown::read(
		rd::SerializationCtx& ctx, rd::Buffer& buffer)
	{
	...
	// intern
	// equals trait
	bool RequestResultBase_Unknown::equals(const ISerializable& object) const
	{
		const auto& other = dynamic_cast<const RequestResultBase_Unknown&>(object);
		if (this == &other)
		{
			return true;
		}
