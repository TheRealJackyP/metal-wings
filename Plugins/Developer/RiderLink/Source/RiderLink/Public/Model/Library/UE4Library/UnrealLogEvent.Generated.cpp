...
namespace JetBrains::EditorPlugin
{
	...
	UnrealLogEvent::UnrealLogEvent(rd::Wrapper<LogMessageInfo> info_, FString text_,
		...
		bpPathRanges_(std::move(bpPathRanges_)), methodRanges_(std::move(methodRanges_))
	{
		initialize();
	}
	...
	UnrealLogEvent UnrealLogEvent::read(rd::SerializationCtx& ctx, rd::Buffer& buffer)
	{
		...
		UnrealLogEvent res{
			std::move(info_), std::move(text_), std::move(bpPathRanges_),
			std::move(methodRanges_)
		};
		return res;
	}
	...
	void UnrealLogEvent::write(rd::SerializationCtx& ctx, rd::Buffer& buffer) const
	{
		...
		buffer.write_array<TArray, StringRange, FDefaultAllocator>(bpPathRanges_,
			[&ctx, &buffer](const StringRange& it) mutable -> void
			{
	...
	// identify
	// getters
	const LogMessageInfo& UnrealLogEvent::get_info() const
	{
		return *info_;
	}

	const FString& UnrealLogEvent::get_text() const
	{
		return text_;
	}

	const TArray<rd::Wrapper<StringRange>>& UnrealLogEvent::get_bpPathRanges() const
	{
		return bpPathRanges_;
	}

	const TArray<rd::Wrapper<StringRange>>& UnrealLogEvent::get_methodRanges() const
	{
	...
	// intern
	// equals trait
	bool UnrealLogEvent::equals(const ISerializable& object) const
	{
		const auto& other = dynamic_cast<const UnrealLogEvent&>(object);
		if (this == &other)
		{
			return true;
		}
	...
	size_t UnrealLogEvent::hashCode() const noexcept
	{
		...
		__r = __r * 31 + (contentDeepHashCode(get_bpPathRanges()));
		__r = __r * 31 + (contentDeepHashCode(get_methodRanges()));
		return __r;
	}
