...
namespace JetBrains::EditorPlugin
{
	...
	public:
		...
		UnrealLogEvent(const UnrealLogEvent&) = default;

		UnrealLogEvent& operator=(const UnrealLogEvent&) = default;

		UnrealLogEvent(UnrealLogEvent&&) = default;

		UnrealLogEvent& operator=(UnrealLogEvent&&) = default;

		virtual ~UnrealLogEvent() override = default;
		...
		// writer
		virtual void
		write(rd::SerializationCtx& ctx, rd::Buffer& buffer) const override;
		...
		// getters
		const LogMessageInfo& get_info() const;
		const FString& get_text() const;
		const TArray<rd::Wrapper<StringRange>>& get_bpPathRanges() const;
		const TArray<rd::Wrapper<StringRange>>& get_methodRanges() const;

		// intern

	private:
		// equals trait
		virtual bool equals(const ISerializable& object) const override;
...
namespace std
{
	template <>
	class tuple_size<
			JetBrains::EditorPlugin::UnrealLogEvent> : public integral_constant<
			size_t, 4>
	{
	};
	...
	{
	public:
		using type = decltype (declval<JetBrains::EditorPlugin::UnrealLogEvent>().get<
			I>());
	};
}
#endif
