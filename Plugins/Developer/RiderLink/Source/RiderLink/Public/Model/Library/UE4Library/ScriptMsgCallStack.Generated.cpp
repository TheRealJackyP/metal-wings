...
namespace JetBrains::EditorPlugin
{
	...
	// primary ctor
	ScriptMsgCallStack::ScriptMsgCallStack(FString message_,
	                                            ...
	                                            rd::Buffer& buffer)
	{
		...
		auto scriptCallStack_ = ctx.get_serializers().readPolymorphic<IScriptCallStack>(
			ctx, buffer);
	...
	void ScriptMsgCallStack::write(rd::SerializationCtx& ctx, rd::Buffer& buffer) const
	{
		...
		ctx.get_serializers().writePolymorphic<IScriptCallStack>(
			ctx, buffer, scriptCallStack_);
	}

	// virtual init
	// identify
	// getters
	const FString& ScriptMsgCallStack::get_message() const
	{
		return message_;
	}

	const IScriptCallStack& ScriptMsgCallStack::get_scriptCallStack() const
	{
	...
	// intern
	// equals trait
	bool ScriptMsgCallStack::equals(const ISerializable& object) const
	{
		const auto& other = dynamic_cast<const ScriptMsgCallStack&>(object);
		if (this == &other)
		{
			return true;
		}
