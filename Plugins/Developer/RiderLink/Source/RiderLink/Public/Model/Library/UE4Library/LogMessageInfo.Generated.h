...
namespace JetBrains::EditorPlugin
{
	...
	public:
		...
		LogMessageInfo(const LogMessageInfo&) = default;

		LogMessageInfo& operator=(const LogMessageInfo&) = default;

		LogMessageInfo(LogMessageInfo&&) = default;

		LogMessageInfo& operator=(LogMessageInfo&&) = default;

		virtual ~LogMessageInfo() override = default;
		...
		// getters
		const ELogVerbosity::Type& get_type() const;
		const FString& get_category() const;
		const rd::optional<rd::DateTime>& get_time() const;
...
namespace std
{
	template <>
	class tuple_size<
			JetBrains::EditorPlugin::LogMessageInfo> : public integral_constant<
			size_t, 3>
	{
	};
	...
	{
	public:
		using type = decltype (declval<JetBrains::EditorPlugin::LogMessageInfo>().get<
			I>());
	};
}
#endif
