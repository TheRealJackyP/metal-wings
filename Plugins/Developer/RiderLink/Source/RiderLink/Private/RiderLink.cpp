...
void FRiderLinkModule::InitProtocol()
{
	...
	std::shared_ptr<rd::SocketWire::Server> Wire = ProtocolFactory::CreateWire(
		&Scheduler, WireLifetime);
	Protocol = ProtocolFactory::CreateProtocol(&Scheduler, WireLifetime.create_nested(),
	                                           Wire);
	...
	//		}
	//	});
	Protocol->wire->connected.view(WireLifetime,
	                               [this](rd::Lifetime ConnectionLifetime,
	                                      const bool& IsConnected)
	                               {
		                               Scheduler.queue(
			                               [this, ConnectionLifetime, IsConnected]()
			                               {
				                               if (!IsConnected)
				                               {
					                               return;
				                               ...
				                               FRWScopeLock LockOnConnect(
					                               ModelLock, SLT_Write);
				                               EditorModel = MakeUnique<
					                               JetBrains::EditorPlugin::RdEditorModel>();
				                               EditorModel->connect(
					                               ConnectionLifetime, Protocol.Get());
                                 ...
                                 )> Handler)
{
	...
	{
		RdIsModelAlive.view(Lifetime,
		                    [this, Handler](rd::Lifetime ModelLifetime,
		                                    const bool& Cond)
		                    {
			                    if (Cond)
			                    {
...
bool FRiderLinkModule::FireAsyncAction(
	TFunction<void(const JetBrains::EditorPlugin::RdEditorModel&)> Handler)
{
