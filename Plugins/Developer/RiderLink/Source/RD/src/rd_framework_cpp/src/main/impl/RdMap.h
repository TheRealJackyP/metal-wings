...
namespace rd
{
	 ...
	 */
	template <typename K, typename V, typename KS = Polymorphic<K>, typename VS =
	          Polymorphic<V>, typename KA = std::allocator<K>,
	          typename VA = std::allocator<V>>
	class RdMap final : public RdReactiveBase, public ViewableMap<K, V, KA, VA>,
	...
	private:
		...
		mutable ordered_map<const K*, int64_t, wrapper::TransparentHash<K>,
		                    wrapper::TransparentKeyEqual<K>> pendingForAck;

		std::string logmsg(Op op, int64_t version, const K* key,
		                   const V* value = nullptr) const
		{
	...
	public:
		...
		virtual ~RdMap() override = default;
		// endregion
		...
		virtual void init(Lifetime lifetime) const override
		{
			...
			local_change([this, lifetime]()
			{
				advise(lifetime, [this, lifetime](Event e)
				{
					...
					const V* new_value = e.get_new_value();
					if (new_value)
					{
					...
					{
						int32_t versionedFlag = ((is_master ? 1 : 0)) <<
							versionedFlagShift;
						Op op = static_cast<Op>(e.v.index());

						buffer.write_integral<int32_t>(
							static_cast<int32_t>(op) | versionedFlag);
						...
						KS::write(this->get_serialization_context(), buffer,
						          *e.get_key());

						const V* new_value = e.get_new_value();
						if (new_value)
						{
							VS::write(this->get_serialization_context(), buffer,
							          *new_value);
						}

						spdlog::get("logSend")->trace(
							"SEND{}",
							logmsg(op, next_version - 1, e.get_key(), new_value));
					});
				});
			});
			...
			{
				this->view(lifetime,
				           [this](Lifetime lf, std::pair<const K*, const V*> entry)
				           {
		...
		virtual void on_wire_received(Buffer buffer) const override
		{
			...
			if (op == Op::ACK)
			{
				...
				else
				{
					if (pendingForAck.count(key) > 0)
					{
						...
						else
						{
							...
							if (pendingVersion == version)
							{
								pendingForAck.erase(key);
								// else we don't need to remove, silently drop
							}
							// return good result
						}
					}
				...
				if (errmsg.empty())
				{
					spdlog::get("logReceived")->trace(
						logmsg(Op::ACK, version, &(wrapper::get<K>(key))));
				}
				else
				{
					spdlog::get("logReceived")->error(
						logmsg(Op::ACK, version, &(wrapper::get<K>(key))) + " >> " +
						errmsg);
				}
			}
			else
			{
				...
				{
					spdlog::get("logReceived")->trace(
						"RECV{}", logmsg(op, version, &(wrapper::get<K>(key)), value));
					if (value.has_value())
					{
				...
				if (msg_versioned)
				{
					...
					{
						spdlog::get("logReceived")->error(
							"Both ends are masters: {}", to_string(location));
					}
				}
			}
		}
		...
		friend std::string to_string(const RdMap& value)
		{
			std::string res = "[";
...
static_assert(std::is_move_constructible_v<rd::RdMap<int, int>>,
	"Is move constructible RdMap<int, int>");
static_assert(std::is_move_assignable_v<rd::RdMap<int, int>>,
	"Is move constructible RdMap<int, int>");
static_assert(std::is_default_constructible_v<rd::RdMap<int, int>>,
