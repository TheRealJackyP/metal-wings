...
namespace rd
{
	...
	private:
		...
		mutable ordered_map<InternedAny, int32_t, any::TransparentHash,
		                    any::TransparentKeyEqual> inverse_map;
...
namespace rd
{
	...
	Wrapper<T> InternRoot::un_intern_value(int32_t id) const
	{
		...
		return any::get<T>(is_index_owned(id)
			                   ? my_items_lis[id / 2]
	...
	int32_t InternRoot::intern_value(Wrapper<T> value) const
	{
		...
		{
			get_protocol()->get_wire()->send(this->rdid,
			                                 [this, &index, value, any](Buffer& buffer)
			                                 {
				                                 InternedAnySerializer::write<T>(
					                                 get_serialization_context(),
					                                 buffer, wrapper::get<T>(value));
				                                 {
					                                 std::lock_guard<decltype(lock)>
						                                 guard(lock);
