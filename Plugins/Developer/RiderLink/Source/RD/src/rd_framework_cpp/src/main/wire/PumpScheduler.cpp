...
namespace rd
{
	namespace test
	{
		namespace util
		{
			PumpScheduler::PumpScheduler() : created_thread_id(
			...
			void PumpScheduler::assert_thread() const
			{
				RD_ASSERT_MSG(created_thread_id == std::this_thread::get_id(),
				              "Illegal thread for current action, must be: " + to_string
				              (created_thread_id) +
			...
			PumpScheduler::PumpScheduler(const std::string& name) : PumpScheduler()
			{
				this->name = name;
			}
		} // namespace util
