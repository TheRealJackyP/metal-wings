...
namespace rd
{
	...
	void RdExtBase::init(Lifetime lifetime) const
	{
		...
		lifetime->bracket(
			...
			{
				extProtocol =
					std::make_shared<Protocol>(parentProtocol->identity, sc,
		...
		lifetime->bracket([this, parentWire]
		                  {
	...
	void RdExtBase::on_wire_received(Buffer buffer) const
	{
		...
		if (serializationHash != counterpartSerializationHash)
		{
			RD_ASSERT_MSG(false, "serializationHash of ext " + to_string(location) +
			              ...
			              "our: " +
			              to_string(serializationHash) + ", counterpart:" + to_string(
				              counterpartSerializationHash))
		}
	}

	void RdExtBase::sendState(const IWire& wire, ExtState state) const
	{
