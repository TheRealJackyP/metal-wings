...
namespace rd
{
	...
	Buffer::Buffer(ByteArray array, size_t offset) : data_(std::move(array)),
	                                                 offset(offset)
	{
	}
	...
	const Buffer::word_t* Buffer::current_pointer() const
	{
		return data() + offset;
	}
	...
	void write_wstring_spec<2>(Buffer& buffer, wstring_view value)
	{
		...
		buffer.write(reinterpret_cast<const Buffer::word_t*>(value.data()),
		             sizeof(wchar_t) * value.size());
	}

	void Buffer::write_wstring(const std::wstring& value)
	{
	...
	void Buffer::write_char16_string(const uint16_t* data, size_t len)
	{
		...
		write(reinterpret_cast<const word_t*>(data), sizeof(uint16_t) * len);
	}
	...
	uint16_t* Buffer::read_char16_string()
	{
		...
		read(reinterpret_cast<word_t*>(&result[0]), sizeof(uint16_t) * len);
		result[len] = 0;
		return result;
	}
	...
	void Buffer::write_wstring(const Wrapper<std::wstring>& value)
	{
		write_wstring(*value);
	}
	...
	DateTime Buffer::read_date_time()
	{
		...
		time_t t = time_in_ticks / TICKS_PER_MILLISECOND - TICKS_AT_EPOCH /
			TICKS_PER_MILLISECOND;
		return DateTime{t};
	}

	void Buffer::write_date_time(const DateTime& date_time)
	{
