...
namespace rd
{
	...
	public:
		...
		RdTaskResult(const RdTaskResult&) = default;

		RdTaskResult(RdTaskResult&&) = default;

		RdTaskResult& operator=(RdTaskResult&&) = default;

		virtual ~RdTaskResult() override = default;
		// endregion
		...
		static RdTaskResult<T, S> read(SerializationCtx& ctx, Buffer& buffer)
		{
			...
			default:
				throw std::invalid_argument(
					"Fail on RdTaskResult reading with kind: " + std::to_string(kind));
			}
		}
		...
		{
			return visit(util::make_visitor([](const Success& value) -> const T&
			                                {
				                                return wrapper::get<T>(value.value);
			                                },
			                                [](const Cancelled&) -> const T&
			                                {
				                                throw std::invalid_argument(
					                                "Task finished in Cancelled state");
			                                },
			                                [](const Fault& value) -> const T&
			                                {
				                                throw std::runtime_error(
					                                to_string(value.reason_message));
			                                }),
			             v);
		}
		...
		void as_canceled(std::function<void(const Cancelled&)> f) const
		{
		...
		void as_faulted(std::function<void(const Fault&)> f)
		{
			f(rd::get<Fault>(v));
		}
		...
		friend std::string to_string(const RdTaskResult& taskResult)
		{
			return visit(util::make_visitor([](const Success& value) -> std::string
			                                {
				                                return to_string(value.value);
			                                },
			                                [](const Cancelled&) -> std::string
			                                {
				                                return "Cancelled state";
			                                },
			                                [](const Fault& value) -> std::string
			                                {
