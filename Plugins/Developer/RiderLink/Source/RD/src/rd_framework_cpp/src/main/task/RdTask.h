...
namespace rd
{
	...
	public:
		...
		RdTask(const RdTask& other) = default;

		RdTask& operator=(const RdTask& other) = default;
		...
		void fault(const std::exception& e) const
		{
		...
		void advise(Lifetime lifetime, std::function<void(const TRes&)> handler) const
		{
			impl->result.advise(lifetime,
			                    [handler = std::move(handler)](
			                    const optional<TRes>& opt_value)
			                    {
				                    if (opt_value)
				                    {
					                    handler(*opt_value);
