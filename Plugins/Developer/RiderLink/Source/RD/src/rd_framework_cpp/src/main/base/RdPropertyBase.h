...
namespace rd
{
	...
	public:
		...
		RdPropertyBase(const RdPropertyBase&) = delete;
		...
		virtual ~RdPropertyBase() override = default;
		// endregion
		...
		virtual void init(Lifetime lifetime) const override
		{
			...
			if (!optimize_nested)
			{
				this->change.advise(lifetime, [this](const T& v)
				{
					if (is_local_change)
					{
						if (this->has_value())
			...
			advise(lifetime, [this](const T& v)
			{
				if (!is_local_change)
				{
					return;
			...
			if (!optimize_nested)
			{
				this->view(lifetime, [this](Lifetime lf, const T& v)
				{
					if (this->has_value())
					{
		...
		virtual void set(value_or_wrapper<T> new_value) const override
		{
...
static_assert(std::is_move_constructible_v<rd::RdPropertyBase<int>>,
