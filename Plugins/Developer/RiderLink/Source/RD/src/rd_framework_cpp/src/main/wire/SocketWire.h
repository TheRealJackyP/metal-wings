	...
	public:
		...
		protected:
			...
			mutable ByteBufferAsyncProcessor async_send_buffer{
				...
				[this](const Buffer::ByteArray& it, sequence_number_t seqn) -> bool
				{
			...
			mutable decltype(receiver_buffer)::iterator
				lo = receiver_buffer.begin(), hi = receiver_buffer.begin();

			static constexpr int32_t ACK_MESSAGE_LENGTH = -1;
			static constexpr int32_t PING_MESSAGE_LENGTH = -2;
			static constexpr int32_t PACKAGE_HEADER_LENGTH = sizeof(ACK_MESSAGE_LENGTH)
				+ sizeof(sequence_number_t);
			...
			{
				return read_from_socket(data, static_cast<int32_t>(len));
			}
		...
		public:
			...
			virtual void send(const RdId& rd_id,
			                  std::function<void(
				                  Buffer& buffer)> writer) const override;
