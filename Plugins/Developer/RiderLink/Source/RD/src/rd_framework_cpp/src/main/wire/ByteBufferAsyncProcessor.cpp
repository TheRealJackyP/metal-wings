...
namespace rd
{
	...
	ByteBufferAsyncProcessor::ByteBufferAsyncProcessor(
		std::string id,
		std::function<bool(const Buffer::ByteArray&, sequence_number_t)> processor)
	...
	bool ByteBufferAsyncProcessor::terminate0(time_t timeout, StateKind state_to_set,
	                                          string_view action)
	{
		{
			std::lock_guard<decltype(lock)> guard(lock);
	...
	{
		{
			...
			{
				const auto& item = pending_queue[i];
	...
	{
		util::set_thread_name(id.empty()
			                      ? "ByteBufferAsyncProcessor Thread"
			                      : id.c_str());
