...
namespace rd
{
			...
			{
				{
					...
					while (true)
					{
						...
						sendQ.pop();
						realWire->send(
							it.first, [payload = std::move(it.second)](Buffer& buffer)
							{
	...
	void ExtWire::advise(Lifetime lifetime, const IRdReactive* entity) const
	{
		realWire->advise(lifetime, entity);
	}

	void ExtWire::send(const RdId& id, std::function<void(Buffer& buffer)> writer) const
	{
		{
			std::lock_guard<decltype(lock)> guard(lock);
