...
namespace rd
{
	...
	public:
		...
		virtual ~RdCall() override = default;
		// endregion

		static RdCall<TReq, TRes, ReqSer, ResSer> read(
		 ...
		 */
		WiredRdTask<TRes, ResSer> sync(const TReq& request,
		                               std::chrono::milliseconds timeout = 200ms) const
		{
			auto task = start_internal(request, true,
			                           &SynchronousScheduler::Instance());
		 ...
		 */
		WiredRdTask<TRes, ResSer> start(const TReq& request,
		                                IScheduler* responseScheduler = nullptr) const
		{
	...
	private:
		WiredRdTask<TRes, ResSer> start_internal(const TReq& request, bool sync,
	...
	public:
		...
		friend std::string to_string(const RdCall& /*value*/)
		{
			return "RdCall";
		}
	};
