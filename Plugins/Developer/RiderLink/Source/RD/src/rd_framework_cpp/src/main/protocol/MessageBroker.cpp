...
namespace rd
{
	...
	void MessageBroker::invoke(const IRdReactive* that, Buffer msg, bool sync) const
	{
		...
		else
		{
			auto action = [this, that, message = std::move(msg)]() mutable
			{
				...
				{
					logger->trace(
						"Disappeared Handler for Reactive entities with id: {}",
						to_string(that->get_id()));
				}
			};
			std::function<void()> function = util::make_shared_function(
				std::move(action));
			that->get_wire_scheduler()->queue(std::move(function));
		}
	}

	MessageBroker::MessageBroker(IScheduler* defaultScheduler) : default_scheduler(
		defaultScheduler)
	{
	}
	...
	void MessageBroker::dispatch(RdId id, Buffer message) const
	{
...

		{
			...
			std::lock_guard<decltype(lock)> guard(lock);
			const IRdReactive* s = subscriptions[id];
			if (s == nullptr)
			{
				...
				auto action = [this, it, id]() mutable
				{
					...
					const IRdReactive* subscription = subscriptions[id];
					...
					if (subscription != nullptr)
					{
						if (message)
						{
							invoke(subscription, *std::move(message),
							       subscription->get_wire_scheduler() ==
							       default_scheduler);
						}
					}
					else
			...
			{
				if (s->get_wire_scheduler() == default_scheduler || s->
					get_wire_scheduler()->out_of_order_execution)
				{
	...
	void MessageBroker::advise_on(Lifetime lifetime, const IRdReactive* entity) const
	{
		RD_ASSERT_MSG(!entity->get_id().isNull(),
		...
		if (!lifetime->is_terminated())
		{
			...
			const IRdReactive* value = entity;
