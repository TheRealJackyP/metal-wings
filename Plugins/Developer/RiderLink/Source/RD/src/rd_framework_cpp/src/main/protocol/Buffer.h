...
namespace rd
{
	...
	public:
		...
		{
			write(reinterpret_cast<const word_t*>(&value), sizeof(T));
		}
		...
		void write_floating_point(const T& value)
		{
			write(reinterpret_cast<const word_t*>(&value), sizeof(T));
		}
		...
		template <template <class, class> class C, typename T, typename A = allocator<T>
		          ,
		          typename = std::enable_if_t<util::is_pod_v<T>>>
		C<T, A> read_array()
		{
		...
		template <template <class, class> class C, typename T, typename A = allocator<
			          value_or_wrapper<T>>>
		C<value_or_wrapper<T>, A> read_array(
			std::function<value_or_wrapper<T>()> reader)
		{
			int32_t len = read_integral<int32_t>();
		...
		template <template <class, class> class C, typename T, typename A = allocator<T>
		          ,
		          typename = std::enable_if_t<util::is_pod_v<T>>>
		void write_array(const C<T, A>& container)
		{
			using rd::size;
			...
			if (len > 0)
			{
				write(reinterpret_cast<const word_t*>(&container[0]), sizeof(T) * len);
			}
		}

		template <template <class, class> class C, typename T, typename A = allocator<T>
		          ,
		          typename = std::enable_if_t<!rd::util::in_heap_v<T>>>
		void write_array(const C<T, A>& container, std::function<void(const T&)> writer)
		{
			using rd::size;
		...
		template <template <class, class> class C, typename T, typename A = allocator<
			          Wrapper<T>>>
		void write_array(const C<Wrapper<T>, A>& container,
		...
		void write_byte_array_raw(const ByteArray& array);
		...
		void write_wstring(const Wrapper<std::wstring>& value);

		DateTime read_date_time();

		void write_date_time(const DateTime& date_time);

		template <typename T, typename = std::enable_if_t<util::is_enum_v<T>>>
		T read_enum()
		{
		...
		template <typename T, typename = std::enable_if_t<util::is_enum_v<T>>>
		void write_enum_set(const T& x)
		{
			write_integral<int32_t>(static_cast<int32_t>(x));
		}

		template <typename T, typename F, typename = std::enable_if_t<util::is_same_v<
			          util::result_of_t<F()>, T>>>
		...
		template <typename T, typename F,
		          typename = std::enable_if_t<util::is_same_v<
			          util::result_of_t<F()>, Wrapper<T>>>>
		...
		template <typename T>
		std::enable_if_t<!std::is_abstract_v<T>> write_nullable(
			const optional<T>& value, std::function<void(const T&)> writer)
		{
			if (!value)
			{
				write_bool(false);
		...
		template <typename T, typename F>
		std::enable_if_t<!util::is_invocable_v<F, Wrapper<T>>> write_nullable(
			const Wrapper<T>& value, F&& writer)
		{
			if (!value)
			{
				write_bool(false);
		...
		const word_t* current_pointer() const;
