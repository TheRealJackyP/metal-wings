...
namespace rd
{
	...
	void RdBindableBase::bind(Lifetime lf, const IRdDynamic* parent,
	                          string_view name) const
	{
		RD_ASSERT_MSG(!is_bound(),
		              ("Trying to bind already bound this to " + to_string(parent->
			              get_location())));
		lf->bracket(
			[this, lf, parent, &name]
			{
	...
	void RdBindableBase::identify(const Identities& identities, const RdId& id) const
	{
