...
namespace rd
{
	...
	class RdList final : public RdReactiveBase, public ViewableList<T, A>,
	...
	public:
		...
		virtual ~RdList() override = default;
		// endregion
		...
		virtual void init(Lifetime lifetime) const override
		{
			...
			local_change([this, lifetime]
			{
				advise(lifetime, [this, lifetime](typename IViewableList<T>::Event e)
				{
					...
					if (!optimize_nested)
					{
						const T* new_value = e.get_new_value();
						if (new_value)
						{
					...
					get_wire()->send(rdid, [this, e](Buffer& buffer)
					{
						...
						buffer.write_integral<int64_t>(
							static_cast<int64_t>(op) | (next_version++ <<
								versionedFlagShift));
						...
						if (new_value)
						{
							S::write(this->get_serialization_context(), buffer,
							         *new_value);
						}
			...
			if (!optimize_nested)
			{
				this->view(lifetime, [this](Lifetime lf, size_t index, const T& value)
				{
			...
			case Op::ADD:
				{
					...
					spdlog::get("logReceived")->trace(
						logmsg(op, version, index, &(wrapper::get<T>(value))));
		...
		friend std::string to_string(const RdList& value)
		{
			std::string res = "[";
...
static_assert(std::is_move_constructible_v<rd::RdList<int>>,
