...
namespace rd
{
	...
	class RdEndpoint : public virtual RdReactiveBase, public ISerializable
	{
		...
		using handler_t = std::function<RdTask<TRes, ResSer>(Lifetime, const TReq&)>;
		mutable handler_t local_handler;

		mutable ordered_map<RdId, RdTask<TRes, ResSer>, hash<RdId>> awaiting_tasks;
		// TO-DO get rid of it
	public:
		// region ctor/dtor
		...
		explicit RdEndpoint(std::function<WTRes(const TReq&)> handler)
		{
			set(std::move(handler));
		}
		...
		virtual ~RdEndpoint() override = default;
		// endregion

		static RdEndpoint<TReq, TRes, ReqSer, ResSer> read(
			SerializationCtx& /*ctx*/, Buffer& buffer)
		{
		 ...
		 * \brief @see set above
		 */
		void set(std::function<WTRes(const TReq&)> functor) const
		{
			local_handler = [handler = std::move(functor)](
				Lifetime _, const TReq& req) -> RdTask<TRes, ResSer>
				{
		...
		virtual void on_wire_received(Buffer buffer) const override
		{
			...
			spdlog::get("logReceived")->trace("endpoint {}::{} request = {}",
			...
			task.advise(*bind_lifetime,
			            [this, task_id, &task](
			            const RdTaskResult<TRes, ResSer>& task_result)
			            {
				            spdlog::get("logSend")->trace(
					            "endpoint {}::{} response = {}", to_string(location),
					            to_string(rdid), to_string(*task.result));
				            get_wire()->send(
					            task_id, [&](Buffer& inner_buffer)
					            {
						            task_result.write(
							            get_serialization_context(), inner_buffer);
					            });
		...
		friend std::string to_string(const RdEndpoint& /*value*/)
		{
			return "RdEndpoint";
		}
	};
