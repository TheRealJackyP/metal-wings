...
namespace rd
{
	template <typename S, template <class, class> class C, typename T = util::read_t<S>,
	...
	public:
		...
		static void write(SerializationCtx& ctx, Buffer& buffer,
		                  const C<value_or_wrapper<T>, A>& value)
		{
			buffer.write_array<C, T, A>(value, [&](const T& inner_value)
			{
