...
namespace rd
{
	...
	void SocketWire::Base::receiverProc() const
	{
		while (!lifetimeDef.lifetime->is_terminated())
		{
			try
			{
				...
				{
					logger->debug(
						"{}: stop receive messages because socket disconnected",
						this->id);
	...
	bool SocketWire::Base::send0(const Buffer::ByteArray& msg,
	                             sequence_number_t seqn) const
	{
		try
		{
			...
			RD_ASSERT_THROW_MSG(
				socket_provider->Send(send_package_header.data(), send_package_header.
					get_position()) == PACKAGE_HEADER_LENGTH,
				this->id +
	...
	void SocketWire::Base::send(const RdId& rd_id,
		...
		while (ptr < msglen)
		{
			...
			else
			{
				...
				if (read == -1)
				{
					...
					{
						logger->info("{}: socket was shut down for receiving",
						             this->id);
						return false;
					}
	...
	void SocketWire::Base::ping() const
	{
		...
		try
		{
			...
			{
				std::lock_guard<decltype(socket_send_lock)> guard(socket_send_lock);
				int32_t sent = socket_provider->Send(
					ping_pkg_header.data(), ping_pkg_header.get_position());
				if (sent == 0 && !socket_provider->IsSocketValid())
				{
					logger->debug(
						"{}: failed to send ping over the network, reason: socket was shut down for sending",
						this->id);
					return;
				}
				RD_ASSERT_THROW_MSG(sent == PACKAGE_HEADER_LENGTH,
				                    fmt::format(
					                    "{}: failed to send ping over the network, reason: {}"
		  ...
		  clientLifetimeDefinition(parentLifetime)
	{
		...
		{
			util::set_thread_name(this->id.empty()
				                      ? "SocketWire::Client Thread"
				                      : this->id.c_str());
			...
			try
			{
				...
				while (!lifetime->is_terminated())
				{
					try
					{
						...
						logger->info("{}: connecting 127.0.0.1: {}", this->id,
						             this->port);
						RD_ASSERT_THROW_MSG(socket->Open("127.0.0.1", this->port),
						                    fmt::format(
							                    "{}: failed to open ActiveSocket, reason: {}"
							                    , this->id, socket->DescribeError()));
						{
							std::lock_guard<decltype(lock)> guard(lock);
					...
					{
						logger->debug("{}: connection error for port {} ({}).",
						              this->id, this->port, e.what());
	...
	SocketWire::Server::Server(Lifetime parentLifetime, IScheduler* scheduler,
	                           uint16_t port, const std::string& id)
		: Base(id, parentLifetime, scheduler), ss(std::make_unique<CPassiveSocket>()),
	...
	signal(SIGPIPE, SIG_IGN);
		...
		RD_ASSERT_MSG(ss->Initialize(),
		              fmt::format("{}: failed to initialize socket, reason: {}", this->
			              id, socket->DescribeError()));
			...
			try
			{
				while (!lifetime->is_terminated())
				{
					try
					{
						...
						RD_ASSERT_THROW_MSG(
							accepted != nullptr,
							fmt::format("{}: accepting failed, reason: {}", this->id, ss
								->DescribeError()));
					...
					{
						logger->info("{}: closed with exception: {}", this->id,
						             e.what());
					}
				}
			}
