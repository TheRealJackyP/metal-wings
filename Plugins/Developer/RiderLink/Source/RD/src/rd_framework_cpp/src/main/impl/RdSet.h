...
namespace rd
{
	...
	class RdSet final : public RdReactiveBase, public ViewableSet<T, A>,
	...
	public:
		...
		virtual ~RdSet() override = default;
		...
		virtual void init(Lifetime lifetime) const override
		{
			...
			{
				advise(lifetime, [this](AddRemove kind, const T& v)
				{
					if (!is_local_change)
					{
						return;
					...
					get_wire()->send(rdid, [this, kind, &v](Buffer& buffer)
					{
						...
						spdlog::get("logSend")->trace(
							"SENDset {} {}:: {}:: {}", to_string(location),
							to_string(rdid), to_string(kind), to_string(v));
					});
				});
			});
		...
		virtual bool add(WT value) const override
		{
		...
		virtual bool remove(const T& value) const override
		{
		...
		friend std::string to_string(const RdSet& value)
		{
			std::string res = "[";
...
static_assert(std::is_move_constructible_v<rd::RdSet<int>>,
