...
namespace rd
{
	...
	{
		optional<InternedAny> value = InternedAnySerializer::read(
			get_serialization_context(), buffer);
		if (!value)
		{
			return;
		}
	...
	void InternRoot::bind(Lifetime lf, const IRdDynamic* parent, string_view name) const
	{
		RD_ASSERT_MSG(!is_bound(),
		              "Trying to bound already bound "s + to_string(this->location) +
		              " to " + to_string(parent->get_location()))
	...
	void InternRoot::identify(const Identities& /*identities*/, const RdId& id) const
	{
	...
	{
		RD_ASSERT_MSG(!is_index_owned(id),
