...
namespace rd
{
	...
	template <typename T>
	struct helper<T, std::enable_if_t<util::in_heap_v<T>>>
	{
	...
	public:
		...
		template <typename R, typename = std::enable_if_t<util::is_base_of_v<T, R>>>
		Wrapper(const Wrapper<R>& other) : Base(
			std::static_pointer_cast<T>(static_cast<std::shared_ptr<R>>(other)))
		{
		}

		template <typename R, typename = std::enable_if_t<util::is_base_of_v<T, R>>>
		Wrapper(Wrapper<R>&& other) : Base(
		...
		Wrapper(F&& value) : Base(
			std::allocate_shared<G>(alloc, std::forward<F>(value)))
		{
		}

		template <typename F>
		Wrapper(const std::shared_ptr<F>& ptr) noexcept : Base(
			std::static_pointer_cast<T>(ptr))
		{
		}

		template <typename F>
		Wrapper(std::shared_ptr<F>&& ptr) noexcept : Base(
			std::static_pointer_cast<T>(std::move(ptr)))
		{
		}

		template <typename U = T, typename R = std::enable_if_t<!std::is_abstract_v<
			          std::decay_t<U>>>>
		...
		template <typename R>
		static Wrapper<T> dynamic(const Wrapper<R>& w)
		{
		...
		{
			bool is_lhs = static_cast<bool>(lhs);
			bool is_rhs = static_cast<bool>(rhs);
			if (is_lhs != is_rhs)
			{
				return false;
			}
		...
		friend std::string to_string(const Wrapper& value)
		{
	...
	namespace wrapper
	{
		...
		template <typename T>
		std::enable_if_t<!util::in_heap_v<T>, T> unwrap(Wrapper<T>&& ptr)
		{
			return std::move(*ptr);
		}

		template <typename T>
		std::enable_if_t<util::in_heap_v<T>, Wrapper<T>> unwrap(Wrapper<T>&& ptr)
		{
	...
	{
		size_t operator()(const Wrapper<T>& value) const noexcept
		{
