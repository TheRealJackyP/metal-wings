...
namespace rd
{
	...
	size_t contentHashCode(const C<T, A>& list) noexcept
	{
		size_t __r = 0;
	...
	size_t contentDeepHashCode(const T& value) noexcept
	{
	...
	std::enable_if_t<std::is_same_v<decltype(T{}.has_value()), bool>, size_t>
	contentDeepHashCode(const T& value) noexcept
	{
	...
	// containers
	template <class T, typename = std::enable_if_t<!std::is_integral_v<T>>>
	std::enable_if_t<std::is_integral_v<remove_all_t<decltype(*begin(T{}))>>, size_t>
	contentDeepHashCode(const T& value) noexcept
	{
