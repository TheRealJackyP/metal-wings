...
namespace rd
{
	...
	public:
		...
		explicit IProperty(const T& value) : IPropertyBase<T>(value)
		{
		}
		...
		virtual ~IProperty() override = default;
		// endregion

		virtual const T& get() const = 0;

	private:
		void advise0(Lifetime lifetime, std::function<void(const T&)> handler,
		             const Signal<T>& signal) const
		{
