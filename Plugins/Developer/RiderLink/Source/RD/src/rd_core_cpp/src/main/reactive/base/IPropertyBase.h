...
namespace rd
{
	...
	public:
		...
		virtual ~IPropertyBase() override = default;
		// endregion

		virtual void advise_before(Lifetime lifetime,
		                           std::function<void(const T&)> handler) const = 0;
		                                     ...
		                                     const T&)> handler) const override
		{
			...
			std::shared_ptr<SequentialLifetimes> seq = std::make_shared<
				SequentialLifetimes>(lf);

			this->advise_before(lf, [lf, seq](const T& /*v*/)
			{
				if (!lf->is_terminated())
				{
			...
			this->advise(lf, [lf, seq, handler](const T& v)
			{
				if (!lf->is_terminated())
				{
					handler(seq->next(), v);
