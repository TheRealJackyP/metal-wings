...
namespace rd
{
	namespace detail
	{
		...
		{
		public:
			SetEvent(AddRemove kind, const T* value) : kind(kind), value(value)
			{
			}
			...
			friend std::string to_string(const SetEvent& e)
			{
	...
	protected:
		...
		mutable unordered_map<Lifetime,
		                      ordered_map<
			                      const T*, LifetimeDefinition, wrapper::TransparentHash
			                      <T>,
	...
	public:
		...
		virtual ~IViewableSet() override = default;
		// endregion

		/**
		 * \brief Represents an addition or removal of an element in the set.
		 */
		using Event = detail::SetEvent<T>;
		...
		virtual void view(Lifetime lifetime,
		                  std::function<void(Lifetime,
		                                     const T&)> handler) const override
		{
			advise(lifetime, [this, lifetime, handler](AddRemove kind, const T& key)
			{
				switch (kind)
				{
				case AddRemove::ADD:
					{
						...
						const auto& it = lifetimes[lifetime].emplace(&key, lifetime);
		...
		virtual bool remove(const T&) const = 0;
...
static_assert(
	std::is_move_constructible_v<rd::IViewableSet<int>::Event>,
