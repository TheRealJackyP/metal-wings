...
namespace rd
{
	...
	public:
		...
		virtual void view(Lifetime lifetime, std::function<void(Lifetime, const T&)
	...
	public:
		virtual ~ISignal() override = default;

		virtual void fire(const T& value) const = 0;

		/**
		 * \brief @code fire specialisation at T=Void
		 */
		template <typename U = T>
		std::enable_if_t<util::is_void<U>> fire() const
		{
			fire(Void{});
		}
	};
