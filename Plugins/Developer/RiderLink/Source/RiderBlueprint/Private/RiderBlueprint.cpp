...
static void AllowSetForeGroundForEditor(
	const JetBrains::EditorPlugin::RdEditorModel& unrealToBackendModel)
{
	static const int32 CurrentProcessId = FPlatformProcess::GetCurrentProcessId();
	try
	{
		const rd::WiredRdTask<bool> Task = unrealToBackendModel.
		                                   get_allowSetForegroundWindow().sync(
			                                   CurrentProcessId);
		if (Task.is_faulted())
		{
...
void FRiderBlueprintModule::StartupModule()
{
	...
	RiderLinkModule.ViewModel(ModuleLifetimeDef.lifetime,
	                          [this](rd::Lifetime ModelLifetime,
	                                 const JetBrains::EditorPlugin::RdEditorModel&
	                                 UnrealToBackendModel)
	                          {
		                          UnrealToBackendModel.get_openBlueprint().advise(
			                          ModelLifetime,
			                          [this, &UnrealToBackendModel](
			                          const JetBrains::EditorPlugin::BlueprintReference&
			                          s)
			                          {
				                          try
				                          {
					                          AllowSetForeGroundForEditor(
						                          UnrealToBackendModel);

					                          auto Window = FGlobalTabmanager::Get()->
						                          GetRootWindow();
					                          if (!Window.IsValid())
					                          {
						                          return;
		                          ...
		                          UnrealToBackendModel.get_isBlueprintPathName().set(
			                          [](const FString& pathName) -> bool
			                          {
				                          return BluePrintProvider::IsBlueprint(
					                          pathName);
			                          });
	                          });
