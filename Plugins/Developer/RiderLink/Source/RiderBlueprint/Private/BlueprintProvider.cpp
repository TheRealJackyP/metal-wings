...
void BluePrintProvider::AddAsset(const FAssetData& AssetData)
{
#if ENGINE_MAJOR_VERSION == 4 && ENGINE_MINOR_VERSION <= 23
...
bool BluePrintProvider::IsBlueprint(const FString& pathName)
{
	return FPackageName::IsValidObjectPath(pathName);
}

void BluePrintProvider::OpenBlueprint(const FString& AssetPathName,
                                      const TSharedPtr<
	                                      FMessageEndpoint, ESPMode::ThreadSafe>&
                                      messageEndpoint)
{
