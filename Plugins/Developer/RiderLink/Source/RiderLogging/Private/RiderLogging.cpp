...
namespace LoggingExtensionImpl
{
		...
		const FString& Str)
	{
		...
		while (Matcher.FindNext())
		{
			Ranges.Emplace(StringRange(Matcher.GetMatchBeginning(),
			                           Matcher.GetMatchEnding()));
		}
		return Ranges;
	}

	static bool SendMessageToRider(
		const JetBrains::EditorPlugin::LogMessageInfo& MessageInfo,
		const FString& Message)
	{
		static const FRegexPattern PathPattern = FRegexPattern(TEXT("(/[\\w\\.]+)+"));
		static const FRegexPattern MethodPattern = FRegexPattern(
			TEXT("[0-9a-z_A-Z]+::~?[0-9a-z_A-Z]+"));

		return IRiderLinkModule::Get().FireAsyncAction(
			[&MessageInfo, &Message](
			const JetBrains::EditorPlugin::RdEditorModel& RdEditorModel)
			{
				const rd::ISignal<JetBrains::EditorPlugin::UnrealLogEvent>& UnrealLog =
	...
	void SendMessageInChunks(FString* Msg,
	                         const JetBrains::EditorPlugin::LogMessageInfo& MessageInfo)
	{
	...
	void ScheduledSendMessage(FString* Msg,
	                          const JetBrains::EditorPlugin::LogMessageInfo&
	                          MessageInfo)
	{
		FString ToSend;
		while (Msg->Split("\n", &ToSend, Msg))
